...
import org.microbridge.servocontrol.ServoControl;
...
public class Server
{
	...
	{
		Log.d("Server", "receive(): data[0]: " + data[0]); 
		Log.d("Server", "data array is: "); 
		for (int i=0; i< data.length; i++ ) 
		{
			Log.d("Server", "byte[" + i + "]" + data[i]); 
		}
		
		ServoControl.doSomething(data[0]);
		// Notify listeners.
		for (ServerListener listener : listeners)
